...
def match_faq2(rec, user_q):
    ...
    # Instead of calculating the embeddings on real time, we pre-calculate them and then load them
    try:
        with open('../data/precalculated_embeddings/{}_embeddings.pickle'.format(rec), 'rb') as handle:
