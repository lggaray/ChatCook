...
from src.aux_functions import extract_steps_properties, extract_steps_properties2, decode_steps, generate_web_user
...
from pywebio.output import put_text, put_image, put_row
...
def inner_loop(recipe, user):
        ...
        for m in mat.split('，'):
            if m != '。' and m != '':
                ...
                if quantity == recipe or quantity == 'unk':
                    ...
                    ans += m + ' ' + quantity + '\n'
    put_text('[{}]'.format(str(datetime.now()).split('.')[0]) + '【答】： 首先需要准备下面这些材料：\n' + ans)
    ...
    while global_stepn != max(steps_dict.keys()):
        step_question = textarea("请输入您的问题", placeholder="如：“下一步”， “上一步”， “重复一下”， 或者直接输入 “第n步”"
                                 , required=True)  # input('用户：')
        # !!!!
        process_question = '[{}]'.format(str(datetime.now()).split('.')[0]) + '【问】：' + step_question
        put_text(process_question)
        ...
        put_text(process_answer)
    if global_stepn == max(steps_dict.keys()):
        put_text('【答】：辛苦了！菜谱已经完成了，慢慢吃。')
...
img = Image.open('../makeathon/zheda_logo.png')
img2 = Image.open('../makeathon/midea-logo.jpg')
img3 = Image.open('../makeathon/ylab_logo.png')


def chatbot():
    unsolved_questions = []
    data_source = 'waibu'  #textarea('请选数据库来源：内部（写 neibu）或者 外部（写waibu）：', required=True)
    #print('data source: ', data_source)
    
    put_row([put_image(img, width='110px', position=-1), put_image(img3, width='220px', position=-1),
             put_image(img2, width='200px', position=-1)], size='35% 45% 20%')

    user = generate_web_user()
    user['db'] = ['']
    user['tool'] = ['']
    user['sn8'] = ['']
    print(user)
    ...
    while True:
        question = textarea("请输入您的问题", placeholder="如：孕妇适合吃哪些菜？", required=True)
...
if __name__ == '__main__':
    unsolved_questions = start_server(chatbot, port=8085, debug=True, host='localhost', remote_access=False)  # chatbot()
